...
class PPORolloutBuffer(BaseBuffer):
    def __init__(
        ...
        num_transitions_per_env = 512
        assert num_transitions_per_env == 512, "num_transitions_per_env must be equal to 512, if not please change to num_transitions_per_env = n_steps"
        # ----------------------------------------------------------------
        ...
        if self.int_rew_source == ModelType.MIMEx:
            self.seq_obs_mimex = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_mimex,
                self.input_dim, device=self.device
            )
            self.seq_next_obs_mimex = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_mimex,
                self.input_dim, device=self.device
            )
            self.seq_act_mimex = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_mimex,
                dtype=th.int64, device=self.device
            )
        if self.use_my_ratio:
            self.seq_obs_ratio = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_ratio,
                self.input_dim, device=self.device
            )
            self.seq_next_obs_ratio = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_ratio,
                self.input_dim, device=self.device
            )
            # self.seq_act_ratio = th.zeros(
            #     num_transitions_per_env, n_envs, self.seq_expl_len_ratio,
            #     action_shape_local, dtype=th.int64, device=self.device
            # )
            self.seq_act_ratio = th.zeros(
                num_transitions_per_env, n_envs, self.seq_expl_len_ratio,
                action_shape_local, device=self.device
            )
    ...
    def add_mimex_ratio(self, seq_obs_mimex, seq_act_mimex, seq_next_obs_mimex, seq_obs_ratio, seq_act_ratio, seq_next_obs_ratio):
        if self.int_rew_source == ModelType.MIMEx:
            self.seq_obs_mimex[self.pos].copy_(seq_obs_mimex) #  = np.array(seq_obs.unsqueeze(0).cpu()).copy()
            self.seq_next_obs_mimex[self.pos].copy_(seq_next_obs_mimex) #  = np.array(seq_next_obs.unsqueeze(0).cpu()).copy()
            self.seq_act_mimex[self.pos].copy_(seq_act_mimex) #  = np.array(seq_act.unsqueeze(0).cpu()).copy()
        if self.use_my_ratio:
            self.seq_obs_ratio[self.pos].copy_(seq_obs_ratio) #  = np.array(seq_obs.unsqueeze(0).cpu()).copy()
            self.seq_next_obs_ratio[self.pos].copy_(seq_next_obs_ratio) #  = np.array(seq_next_obs.unsqueeze(0).cpu()).copy()
            self.seq_act_ratio[self.pos].copy_(seq_act_ratio) #  = np.array(seq_act.unsqueeze(0).cpu()).copy()
