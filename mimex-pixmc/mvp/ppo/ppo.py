...
class PPO:
    ...
    def __init__(
        ...
        self.seq_expl_len_ratio = expl_cfg["expl_seq_len_ratio"]
        ...
        self.seq_obs_buffer = torch.zeros(
            ...
            dtype=torch.float,
            device=self.device)
        self.seq_next_obs_buffer = torch.zeros(
            self.vec_env.num_envs,
            self.seq_expl_len,
            expl_cfg["input_dim"],
            dtype=torch.float,
            device=self.device)
        self.seq_act_buffer = torch.zeros(
            self.vec_env.num_envs,
            self.seq_expl_len,
            *self.action_space.shape,
            dtype=torch.float,
        ...
        if self.use_my_ratio:
            self.seq_obs_buffer_ratio = torch.zeros(
                self.vec_env.num_envs,
                self.seq_expl_len_ratio,
                expl_cfg["input_dim"],
                dtype=torch.float,
                device=self.device)
            self.seq_next_obs_buffer_ratio = torch.zeros(
                self.vec_env.num_envs,
                self.seq_expl_len_ratio,
                expl_cfg["input_dim"],
                dtype=torch.float,
                device=self.device)
            self.seq_act_buffer_ratio = torch.zeros(
                self.vec_env.num_envs,
                self.seq_expl_len_ratio,
                *self.action_space.shape,
                dtype=torch.float,
        ...
        # self.anneal_k_ratio = expl_cfg["anneal_k_ratio"]
    ...
    def run(self, num_learning_iterations, log_interval=1):
        current_obs = self.vec_env.reset() # [512, 3, 224, 224]
        current_states = self.vec_env.get_state() # [512, 46]
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for nt in range(self.num_transitions_per_env):
                    if self.apply_reset:
                        ...
                        self.seq_next_obs_buffer *= 0.
                        self.seq_act_buffer *= 0.
                        if self.use_my_ratio:
                            self.seq_obs_buffer_ratio *= 0.
                            self.seq_next_obs_buffer_ratio *= 0.
                            self.seq_act_buffer_ratio *= 0.

                    # Compute the action
                    step = it * self.num_transitions_per_env + nt
                    actions, actions_log_prob, values, mu, sigma, current_obs_feats, seq_obs_feats = \
                        self.actor_critic.act(current_obs, current_states, step, return_seq_obs=self.expl_input_type) # actions:[512, 23]
                    # Step the vec_environment
                    next_obs, rews, dones, infos = self.vec_env.step(actions) # actions:[512, 23] # next_obs:[512, 3, 224, 224]
                    next_states = self.vec_env.get_state() # next_states:[512, 46]
                    # Record the transition
                    obs_in = current_obs_feats if current_obs_feats is not None else current_obs # obs_in:[512, 384]

                    if self.seq_expl_len < 1:
                        seq_obs = None
                        # assert self.seq_expl_len >= 1
                    elif self.seq_expl_len == 1:
                        # self.seq_obs_buffer not needed
                        seq_obs = seq_obs_feats.unsqueeze(-2) # seq_obs:[512, 1, 384]

                        # ----------------------------------------------------------------
                        if self.use_my_ratio:
                            self.seq_obs_buffer_ratio[:, 0:-1] = self.seq_obs_buffer_ratio[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                            self.seq_obs_buffer_ratio[:, -1] = seq_obs_feats # seq_obs_feats:[512, 384]
                            seq_obs_ratio = self.seq_obs_buffer_ratio

                            new_ids = (dones > 0).nonzero(as_tuple=False)
                            self.seq_obs_buffer_ratio[new_ids] *= 0
                            # ----------------------------------------------------------------
                            seq_next_obs_feats = self.actor_critic.obs_enc_functions(next_obs, next_states, return_seq_obs=self.expl_input_type)
                            self.seq_next_obs_buffer_ratio[:, 0:-1] = self.seq_next_obs_buffer_ratio[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                            self.seq_next_obs_buffer_ratio[:, -1] = seq_next_obs_feats # seq_obs_feats:[512, 384]
                            ...
                            self.seq_act_buffer_ratio[:, -1] = actions # seq_obs_feats:[512, 384]
                        ...
                        self.seq_obs_buffer[:, 0:-1] = self.seq_obs_buffer[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                        self.seq_obs_buffer[:, -1] = seq_obs_feats # seq_obs_feats:[512, 384]
                        ...
                        # ----------------------------------------------------------------
                        seq_next_obs_feats = self.actor_critic.obs_enc_functions(next_obs, next_states, return_seq_obs=self.expl_input_type)
                        self.seq_next_obs_buffer[:, 0:-1] = self.seq_next_obs_buffer[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                        self.seq_next_obs_buffer[:, -1] = seq_next_obs_feats # seq_obs_feats:[512, 384]
                        seq_next_obs = self.seq_next_obs_buffer

                        new_ids = (dones > 0).nonzero(as_tuple=False)
                        self.seq_next_obs_buffer[new_ids] *= 0
                        # ----------------------------------------------------------------
                        self.seq_act_buffer[:, 0:-1] = self.seq_act_buffer[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                        self.seq_act_buffer[:, -1] = actions # seq_obs_feats:[512, 384]
                        seq_act = self.seq_act_buffer

                        new_ids = (dones > 0).nonzero(as_tuple=False)
                        self.seq_act_buffer[new_ids] *= 0
                        # ----------------------------------------------------------------
                        if self.use_my_ratio:
                            self.seq_obs_buffer_ratio[:, 0:-1] = self.seq_obs_buffer_ratio[:, 1:] # self.seq_obs_buffer:[512, 5, 384]
                            self.seq_obs_buffer_ratio[:, -1] = seq_obs_feats # seq_obs_feats:[512, 384]
                    ...
                    if self.use_my_ratio and self.seq_expl_len == 1:
                        self.storage.add_transitions(
                            obs_in, current_states, actions, rews, dones, values,
                            actions_log_prob, mu, sigma, seq_obs, None, None, seq_obs_ratio, seq_act_ratio, seq_next_obs_ratio
                        )
                    if self.use_my_ratio and self.seq_expl_len is not 1:
                        self.storage.add_transitions(
                            obs_in, current_states, actions, rews, dones, values,
                            actions_log_prob, mu, sigma, seq_obs, seq_act, seq_next_obs, seq_obs_ratio, seq_act_ratio, seq_next_obs_ratio
                        )
                    if not self.use_my_ratio and self.seq_expl_len > 1:
                        self.storage.add_transitions(
                            obs_in, current_states, actions, rews, dones, values,
                            actions_log_prob, mu, sigma, seq_obs, seq_act, seq_next_obs, None, None, None
                        )
                    if not self.use_my_ratio and self.seq_expl_len < 1:
                        self.storage.add_transitions(obs_in, current_states, actions, rews, dones, values,
                            actions_log_prob, mu, sigma, seq_obs, None, None, None, None, None)
                    if not self.use_my_ratio and self.seq_expl_len == 1:
                        self.storage.add_transitions(obs_in, current_states, actions, rews, dones, values,
                            actions_log_prob, mu, sigma, seq_obs, None, None, None, None, None)
                ...
                if self.anneal_k:
                    ...
                    anneal_weight = 1.
                mean_expl_r, mean_ratio = self.storage.compute_expl_r(anneal_weight) # 修改这个函数即可
                ...
                if self.print_log:
                    ...
                    self.log(
                        ...
                        mean_expl_r, rewbuffer_mean, lenbuffer_mean, successbuffer_mean, mean_ratio
                    )
    ...
    def log(
        ...
        mean_trajectory_length, mean_reward, mean_expl_r, rewbuffer_mean, lenbuffer_mean, successbuffer_mean, mean_ratio, width=80, pad=35
    ):
        ...
        self.writer.add_scalar('Train-iter/mean_ratio', mean_ratio, it)
        ...
        log_string = (f"""{'#' * width}\n"""
                      ...
                      f"""{'Mean episode ratio/episode:':>{pad}} {mean_ratio:.4f}\n""")
