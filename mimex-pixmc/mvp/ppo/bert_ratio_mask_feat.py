...
from mvp.ppo.vision_transformer import Block, Block_origin
...
class BERT_RATIO_MASK_FEAT(nn.Module):
    """
    MAE-like BERT.
    """
    ...
    def random_masking(self, x):
        ...
        num_features_to_mask = int(D * (1 - self.mask_ratio)) # The number of features in the mask

        noise = torch.rand(N, L, D, device=x.device)  # Generate noise matrix

        # sort noise for each feature dimension
        ids_shuffle = torch.argsort(noise, dim=-1)  # ascend: small is keep, large is remove

        # keep the first subset of features
        ids_keep = ids_shuffle[:, :, num_features_to_mask:] # Retain unmasked feature indexes
        mask = torch.ones_like(x)
        mask.scatter_(-1, ids_keep, 0) # Set the reserved feature index position to 1

        x_masked = x * mask # Masked features
    ...
    def forward(self, x, seq_act, seq_next_obs, keep_batch=False):
        ...
        z_maks, mask = self.random_masking(x_no_cls)

        # latent = torch.cat((cls_token, z_maks), dim=1) # latent[16, 2, 8]

        pred = self.forward_decoder(z_maks)
